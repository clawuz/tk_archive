...
// Used only by authorizeDrivePreviews.cjs. `redirectUri`, when provided,
// overrides the client's constructor-time redirect URI — needed because a
// Desktop app OAuth client's registered redirect is just `http://localhost`
// (no fixed port), while the interactive script binds an ephemeral port at
// runtime and must advertise that exact loopback URI to Google.
function getAuthUrl(oAuth2Client, redirectUri) {
  const options = { access_type: 'offline', scope: SCOPES, prompt: 'consent' };
  if (redirectUri) options.redirect_uri = redirectUri;
  return oAuth2Client.generateAuthUrl(options);
}

async function saveToken(oAuth2Client, code, redirectUri) {
  const options = redirectUri ? { code, redirect_uri: redirectUri } : { code };
